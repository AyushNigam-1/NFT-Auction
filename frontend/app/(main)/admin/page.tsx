...
const VerificationDashboard = () => {
    ...
    return (
        <div className="flex-1 text-gray-200 font-mono space-y-4 ">
            {/* Header */}
            <Header title="Requests" refetch={refetch} isFetching={isFetching} setSearchQuery={setSearchQuery} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                ${req.status === 'pending' ? 'bg-yellow-900/20 text-yellow-500 border border-yellow-800' : ''}
                ${req.status === 'approved' ? 'bg-green-900/20 text-green-500 border border-green-800' : ''}
                                ${req.status === 'rejected' ? ' bg-red-900/20 text-red-500 border border-red-800' : ''}
              ...
              `}>
                        ...
                        <div className=" flex  gap-3">
                            {req.status != "rejected" &&
                                < button className="flex w-full items-center justify-center  gap-2 bg-red-900/20 text-red-400 hover:bg-red-900/40 py-2 rounded-lg text-sm transition-colors" onClick={() => reviewVerification.mutate({ id: req.id, payload: { approve: false, review_reason: "" } })} >
                                    {(reviewVerification.isPending && !reviewVerification.variables.payload.approve) ? <Loader /> : <XCircle size={16} />}
                                    Reject
                                </button>
                            }

                            {req.status != "approved" && <button className="flex items-center justify-center gap-2 bg-green-900/20 text-green-400 hover:bg-green-900/40 w-full py-2 rounded-lg text-sm transition-colors" onClick={() => reviewVerification.mutate({ id: req.id, payload: { approve: true, review_reason: "" } })}>
